...
def dev(model, config, tokenizer, label2id, device, ckpt_path=None, epoch=None, global_step=None, logger=None,
    ...
    dev_dataloader = choose_dataloader(config, examples, label2id, tokenizer, device,type_)
        ...
        for step, batch_data in tqdm(enumerate(dev_dataloader), total=len(dev_dataloader),
            ...
            if config.data_format == 'single':
                if 'gcn' in config.model_name or 'gat' in config.model_name:
                    if config.scheme in [40,-40,45,-45]:
                        input_ids, token_type_ids, attention_masks, e1_mask, e2_mask, labels,ent_pair,neg_edge_index = batch_data
                        loss, logits = model(input_ids, token_type_ids, attention_masks, labels, e1_mask, e2_mask,ent_pair,neg_edge_index)
                else:
            ...
            if config.l1_lambda!=0:
                ...
                for param in model.parameters():
                    reg_loss += l1_loss(param, torch.zeros_like(param))
                loss =loss +lambda_l1*reg_loss
            loss = loss.mean()
