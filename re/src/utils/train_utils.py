...
from src.dataset_utils.entity_type_marker import NormalDataset,NEGNormalDataset,ALLNEGNormalDataset
...
from src.models.abs_entitymarker_model import ABSSingleEntityMarkersREModel
from src.models.new_abs_entitymarker_model import NEWABSSingleEntityMarkersREModel
from src.models.all_abs_entitymarker_model import ALLABSSingleEntityMarkersREModel
from src.models.neg_entitymarker_model import NEGSingleEntityMarkersREModel
from src.models.all_neg_entitymarker_model import ALLNEGSingleEntityMarkersREModel
...
from src.models.gpnn_entitymarker_model import gpnn_SingleEntityMarkersREModel
from src.models.bioelectra_gpnn_entitymarker_model import bioegre_SingleEntityMarkersREModel
from src.models.spacy_gat_entitymarker_model import spacy_gat_SingleEntityMarkersREModel
from src.models.spacy_neg_gat_entitymarker_model import spacy_neg_gat_SingleEntityMarkersREModel
...
def choose_dataloader(config,examples,label2id,tokenizer,device,type_='train'):
    ...
    #     train_dataset = AbsNormalDataset(examples, config=config, label2id=label2id, tokenizer=tokenizer, device=device)
        ...
        train_dataset = ALLNEGNormalDataset(examples, config=config, label2id=label2id, tokenizer=tokenizer, device=device)
    # elif config.data_format == 'all_abs_single':
    #     train_dataset = ALLAbsNormalDataset(examples, config=config, label2id=label2id, tokenizer=tokenizer, device=device)

    else:
        raise ValueError
    # if 'gcn' in config.model_name:
    #     if type_ == 'train' and config.dataset_name in ['IEPA','BioInfer','AIMed','DDI2013','HPRD-50']:
    #         sampler = set_sample(examples)
    #         train_dataloader = DataLoader(dataset=train_dataset, shuffle=False,collate_fn=train_dataset.collate_fn,
    #                                     num_workers=0, batch_size=config.batch_size,sampler=sampler)
    #     else:
    #         train_dataloader = DataLoader(dataset=train_dataset, shuffle=False,collate_fn=train_dataset.collate_fn,
...
def choose_model(config):
    ...
    elif config.model_name == 'abs_single_entity_maker':
        model = ABSSingleEntityMarkersREModel(config)
    elif config.model_name =='new_abs_single_entity_marker':
        model =NEWABSSingleEntityMarkersREModel(config)
    elif config.model_name == 'all_abs_single_entity_marker':
        model = ALLABSSingleEntityMarkersREModel(config)
    elif config.model_name =='neg_single_entity_marker':
        model = NEGSingleEntityMarkersREModel(config)
    elif config.model_name =='all_neg_single_entity_marker':
        model = ALLNEGSingleEntityMarkersREModel(config)
    ...
    elif config.model_name == 'gpnn_single_entity_marker':
        model = gpnn_SingleEntityMarkersREModel(config)
    elif config.model_name == 'bioelectra_gpnn_entitymarker_model':
        model = bioegre_SingleEntityMarkersREModel(config)
    elif config.model_name == 'spacy_single_entity_marker':
        model = spacy_gat_SingleEntityMarkersREModel(config)
    elif config.model_name =='spacy_gat_single_entity_marker':
        model = spacy_neg_gat_SingleEntityMarkersREModel(config)
    else:
        raise ValueError
    return model
