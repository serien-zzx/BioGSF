...
class NEGNormalDataset(Dataset):
    ...
    def __init__(self, examples, config: BertConfig, tokenizer, label2id, device):
        super(ALLNEGNormalDataset, self).__init__()
        self.config = config
        self.examples = examples
    ...
    def collate_fn(self, features):
        ...
        for example in features:
            ...
            if len(subword_tokens) > self.config.max_len - self.config.total_special_toks:
                ...
                if len(subword_tokens) > self.config.max_len - self.config.total_special_toks:
                    continue
            neg_subword_tokens = self.tokenizer.tokenize(neg_sent)
            neg_num = len(re.findall('(\[dep\])',neg_sent))
            if len(neg_subword_tokens) > self.config.max_len - neg_num:
                logger.info('长度为{},开始裁剪长度'.format(len(neg_subword_tokens)))
                neg_sent = self._process_dep_seq_len(neg_sent)
                if not neg_sent:
                    logger.warning('此数据难以裁剪，进行抛弃......')
                    continue
                neg_subword_tokens = self.tokenizer.tokenize(neg_sent)
                logger.info('裁剪之后的长度为{}'.format(len(neg_subword_tokens)))
                if len(neg_subword_tokens) > self.config.max_len - neg_num:
                    continue

            if batch_max_len < len(subword_tokens):
                batch_max_len = len(subword_tokens)
            if batch_max_len <len(neg_subword_tokens):
                batch_max_len = len(neg_subword_tokens)
            batch_labels.append(label)

            encoder_res = self.tokenizer.encode_plus(subword_tokens, truncation=True, max_length=self.max_len,
                                                     add_special_tokens=True)

            neg_encoder_res = self.tokenizer.encode_plus(neg_subword_tokens, truncation=True, max_length=self.max_len,
                                                     add_special_tokens=True)
            ...
            neg_input_ids = neg_encoder_res['input_ids']
            neg_token_type_ids = neg_encoder_res['token_type_ids']
            neg_attention_mask = neg_encoder_res['attention_mask']

            dep_mask = np.zeros(len(neg_input_ids))
            flag = False
            for i,tag in enumerate(neg_input_ids):
                if tag == self.config.dep_tag_id:
                    dep_mask[i] = 1
            ...
            batch_dep_mask.append(dep_mask)
        ...
        batch_neg_input_ids = torch.tensor(sequence_padding(batch_neg_input_ids, length=pad_length), device=self.device).long()
        batch_neg_token_type_ids = torch.tensor(sequence_padding(batch_neg_token_type_ids, length=pad_length),
                                            device=self.device).long()
        batch_neg_attention_masks = torch.tensor(sequence_padding(batch_neg_attention_masks, length=pad_length),
                                             device=self.device).long()
        batch_dep_mask = torch.tensor(sequence_padding(batch_dep_mask, length=pad_length), device=self.device).long()




        return batch_input_ids, batch_token_type_ids, batch_attention_masks, batch_e1_mask, batch_e2_mask, batch_labels,batch_input_ids,batch_neg_token_type_ids,batch_neg_attention_masks,batch_dep_mask
    ...
    def _process_dep_seq_len(self, text):
        """
            裁切句子的方法，直接使用clinicalTransformer提供的方法
            This function is used to truncate sequences with len > max_seq_len
            Truncate strategy:
        ...
        """
        loop_count = 0
        neg_num = len(re.findall('(\[dep\])',text))
        while len(self.tokenizer.tokenize(text)) > (self.config.max_len - neg_num):
            text = self._truncate_dep_helper(text)
            loop_count += 1
            if loop_count > 50:
                return
        return text

    def _truncate_dep_helper(self, text):
        '''
        这是一个句子一个句子的找
        这里对原始的的text进行去除，并不是tokenize之后的....
        :param text:
        ...
        for (idx,tk) in enumerate(tokens):
            if tk.lower() == self.config.dep_tag:
                spec_tag_idx1 = idx if flag==False else spec_tag_idx1
                flag = True
                spec_tag_idx2 = idx
        start_idx, end_idx = 0, len(tokens) - 1
        truncate_space_head = spec_tag_idx1 - start_idx
...
#         return " ".join(tokens)
    ...
    def __init__(self, examples, config: BertConfig, tokenizer, label2id, device):
        super(MultiNEGNormalDataset, self).__init__()
        self.config = config
        self.examples = examples
    ...
    def collate_fn_predicate(self, features):
        ...
        for example in features:
            ...
            # e2_type = example.ent2_type
            # 把所有的整合在一起
            if self.config.scheme in [31,-31,27,-26,-35,36,-36,37,-37,38,-38,61]:
                all_e1_neg = e1_neg.split(' ')
                all_e2_neg = e2_neg.split(' ')
                for word in all_e2_neg:
                    if word not in all_e1_neg:
            ...
            subword_tokens_1 = self.tokenizer.tokenize(e1_neg)
            subword_tokens_2 = self.tokenizer.tokenize(e2_neg)
            # neg_sent_list = [e1_neg,e2_neg]
            # neg_subword_tokens_list = [subword_tokens_1,subword_tokens_2]
            # for (i,neg) in enumerate(neg_subword_tokens_list):
            #     if len(neg) > self.config.neg_len:
            #         neg_sent = neg_sent_list[i][:self.config.neg_len]
            #         logger.info('长度为{},开始裁剪长度'.format(len(neg)))
            #         neg_subword_tokens = self.tokenizer.tokenize(neg_sent)
            #         neg_subword_tokens_list[i] = neg_subword_tokens
            #         logger.info('裁剪之后的长度为{}'.format(len(neg_subword_tokens)))
            
            # subword_tokens_1 = neg_subword_tokens_list[0]
            # subword_tokens_2 = neg_subword_tokens_list[1]
        ...
        # batch_type = torch.tensor(batch_type,device=self.device).long()
        if 'gcn' in self.config.model_name or 'gat' in self.config.model_name:
            # if self.config.scheme in [39]:
            #     return  batch_input_ids, batch_token_type_ids, batch_attention_masks, batch_e1_mask, batch_e2_mask, batch_labels,batch_e1_neg_input_ids,batch_e1_neg_token_type_ids,batch_e1_neg_attention_masks,batch_e2_neg_input_ids,batch_e2_neg_token_type_ids,batch_e2_neg_attention_masks,batch_e1_neg_mask,batch_e2_neg_mask,batch_ent,batch_type
            if self.config.scheme in [61]:
                edge_index,dep_matrix = make_graph_for_neg(self.tokenizer,pad_length,batch_input_ids.tolist(),self.nlp,self.config.dep_dict,device=self.device)
                return batch_input_ids, batch_token_type_ids, batch_attention_masks, batch_e1_mask, batch_e2_mask,(batch_e1_neg_input_ids,batch_e1_neg_attention_masks,batch_e1_neg_token_type_ids),batch_e1_neg_mask,batch_ent,edge_index,dep_matrix
            else:
                if self.config.scheme in [38,-38]:
                    batch_edge_index = make_graph_for_spacy(self.tokenizer,pad_length,batch_input_ids.tolist(),self.nlp)
                    batch_edge_index = torch.tensor(batch_edge_index,device=self.device).long()
                    return batch_input_ids, batch_token_type_ids, batch_attention_masks, batch_e1_mask, batch_e2_mask,batch_e1_neg_input_ids,batch_e1_neg_token_type_ids,batch_e1_neg_attention_masks,batch_e2_neg_input_ids,batch_e2_neg_token_type_ids,batch_e2_neg_attention_masks,batch_e1_neg_mask,batch_e2_neg_mask,batch_ent,batch_edge_index
                return  batch_input_ids, batch_token_type_ids, batch_attention_masks, batch_e1_mask, batch_e2_mask,batch_e1_neg_input_ids,batch_e1_neg_token_type_ids,batch_e1_neg_attention_masks,batch_e2_neg_input_ids,batch_e2_neg_token_type_ids,batch_e2_neg_attention_masks,batch_e1_neg_mask,batch_e2_neg_mask,batch_ent
    ...
    def collate_fn(self, features):
        ...
        return batch_input_ids, batch_token_type_ids, batch_attention_masks, batch_e1_mask, batch_e2_mask, batch_labels,batch_rel_type,batch_e1_neg_input_ids,batch_e1_neg_token_type_ids,batch_e1_neg_attention_masks,batch_e1_neg_mask,batch_ent
