...
class gcn_NEGSingleEntityMarkersREModel(EntityMarkerBaseModel):
    def __init__(self, config: BertConfig):
        super(gcn_NEGSingleEntityMarkersREModel, self).__init__(config)
        ...
        self.double_ent_fc_layer = FCLayer(self.bert_config.hidden_size*2, self.entity_dim, self.config.dropout_prob)
        self.mid_size = 128

        if config.scheme in [31,-31]:
        ...
        if config.scheme == -31:
            self.input_size = self.bert_config.hidden_size*3
        elif config.scheme ==31:
            self.input_size = self.bert_config.hidden_size*3
        else:
            self.input_size = self.bert_config.hidden_size*2
        self.output_size = 64
        ...
        self.GCN_layer_1 = GraphConvolution(self.input_size,self.mid_size)
    ...
    def init_layer(self):
        ...
        nn.init.xavier_normal_(self.double_ent_fc_layer.linear.weight)
        nn.init.constant_(self.double_ent_fc_layer.linear.bias, 0.)
    ...
    def forward(self, input_ids, token_type_ids, attention_masks, e1_mask, e2_mask ,labels,e1_neg_input_ids, e1_neg_token_type_ids, e1_neg_attention_masks ,e2_neg_input_ids, e2_neg_token_type_ids, e2_neg_attention_masks,e1_neg_mask,e2_neg_mask,ent_pair):
        """
        ...
        edge_index = torch.tensor(self.make_gcn_graph(ent_pair),dtype=torch.float).to(input_ids.device)
        ...
        concat_h = self.get_pool_output(sequence_output, pooled_output, input_ids, e1_mask, e2_mask,attention_mask=attention_masks,front_next_outputs = edge_index,e1_front_outputs=e1_neg_sequence_output,e2_front_outputs=e2_neg_sequence_output,e1_next_outputs=e1_neg_mask,e2_next_outputs=e2_neg_mask)
