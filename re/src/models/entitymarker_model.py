...
class SingleEntityMarkersREModel(EntityMarkerBaseModel):
    def __init__(self, config: BertConfig,scheme=1):
        ...
        self.double_fc_layer = FCLayer(self.bert_config.hidden_size*2, 64 ,self.config.dropout_prob)
    ...
    def forward(self, input_ids, token_type_ids, attention_masks, labels, e1_mask, e2_mask,ent_type=None):
        """
        ...
        if self.config.scheme in [1,2]:
            # ent_type_attention_mask = torch.ones_like(ent_type)
            # ent_type_token_type_ids = torch.zeros_like(ent_type)
            # set_trace()
            # ent_type_output = self.bert_model(ent_type,attention_mask=ent_type_attention_mask,token_type_ids=ent_type_token_type_ids)[0]
            ent_type_output = self.bert_model(ent_type)[0]
            concat_h = self.get_pool_output(sequence_output, pooled_output, input_ids, e1_mask, e2_mask,attention_mask=attention_masks,ent_type=ent_type_output)
        elif self.config.scheme == 5:
            
            concat_h = pooled_output
        else:
